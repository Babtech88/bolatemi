...
- **Orders & Payments**: cart → checkout → Paystack → verified payment → stock
  decrement → order confirmation email with an itemized **PDF receipt attached**,
  sent immediately (synchronously, not queued) as soon as payment is confirmed.
  Confirmed via **both** the redirect-verify route and a signed Paystack
  **webhook** (`/api/webhooks/paystack`) — the webhook is the reliable path in
  production since a closed tab can miss the redirect. No receipt is sent if
  the customer left email blank at checkout (nothing to send it to).
