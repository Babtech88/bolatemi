...
export async function confirmPayment(reference: string) {
  ...
  await finalizeOrderPayment(payment.orderId, payment.order.customer.email ?? undefined, payment.order.orderNumber, Number(payment.amount));
...
async function finalizeOrderPayment(orderId: string, customerEmail: string | undefined, orderNumber: string, total: number) {
  if (customerEmail) {
    await sendMail({
      to: customerEmail,
      subject: `Order confirmed — ${orderNumber}`,
      html: orderConfirmationEmail({ orderNumber, total: `₦${total.toLocaleString()}` }),
    });
  }
}
